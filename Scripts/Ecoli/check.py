...
fasta_sequences = list(SeqIO.parse('Ecoli.faa', 'fasta'))
...
for p in protein_tcdb_sequences:
    for f in fasta_sequences:    
        if str(f.seq) == str(p.seq):
            transporters_coli.append(f.id)
            break


transporters_coli = list(set(transporters_coli))

features_data = pd.read_csv('ecoli-final.csv', sep=',')
count = 0
transp = []
...
for fdata in features_data.values:
    ...
    if fdata_id in transporters_coli: # Aqui calcula com base na condição de ser positivo - transportador
...
print("Ecoli matriz de confusão valores por modelo")
print(f"Total de sequencias: {len(fasta_sequences)}")
print(f"Total de sequencias transportadoras: {len(transporters_coli)}")
